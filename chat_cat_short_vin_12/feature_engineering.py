...
DEFAULT_STATS_PATH = os.path.join(_parent, 'chat_cat_short_vin_12', 'models', 'prefix_statistics.pkl')
...
def extract_features(chassis_series: pd.Series, prefix_stats: dict = None) -> pd.DataFrame:
    """
    Extracts features from a pandas Series of 12-character short chassis numbers.
    """
    logger.info("Extracting features from 12-character chassis numbers...")
    ...
    # 6. ASCII Features: ascii_char_1 to ascii_char_12
    for i in range(12):
    ...
    # Prefix frequencies: map counts from prefix_stats if available (calculated for prefix_3 to prefix_6)
    for length in [3, 4, 5, 6]:
        # Prefix overall purity (confidence score)
