...
def extract_features(chassis_series: pd.Series) -> pd.DataFrame:
    """
    Extracts features from a pandas Series of 11-character short chassis numbers.
    """
    ...
    # 1. Character features: char_1 to char_11
    for i in range(11):
    ...
    # 6. ASCII Features: ascii_char_1 to ascii_char_11
    for i in range(11):
    ...
    features["bigram_4"] = normalized_series.apply(lambda x: x[3:5] if len(x) >= 5 else "??")
    ...
    features["trigram_3"] = normalized_series.apply(lambda x: x[2:5] if len(x) >= 5 else "???")
    ...
    # Extra indices for compatibility / quality in 11-char model
