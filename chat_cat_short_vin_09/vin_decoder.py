...
class VINDecoder:
    def __init__(self, model_dir="chat_cat_short_vin_09/models"):
    ...
    def predict(self, chassis_number: str) -> dict:
        ...
        if not normalized or len(normalized) != 9:
            raise ValueError("Chassis number must be exactly 9 characters.")
            
        # 1. Base Feature Extraction
        # Pass prefix_stats to enable frequency and purity encoding
        X_in = extract_features(pd.Series([normalized]), prefix_stats=self.prefix_stats)
        ...
        for target in ["color", "weight", "cylinders", "no_of_passengers"]:
            if target in self.models:
                ...
                elif target == "color" or target == "weight":
                    X_base["make"] = predictions["make"]
                    X_base["model"] = predictions["model"]
        ...
        output = {
            ...
            "year": str(predictions["year"]),
            "body_type": predictions["body_type"],
            "origin": predictions["origin"],
            "regional_spec": predictions["regional_specs"],
            ...
            "weight": str(int(predictions["weight"])),
            "confidence_scores": {
                "make": round(confidences.get("make", 0.0), 2),
                "model": round(confidences.get("model", 0.0), 2),
                "year": round(confidences.get("year", 0.0), 2)
            }
        }
        return output
