...
def run_analysis(data_path: str, output_dir: str):
    ...
    # 3. Analyze prefix purity for prefix_2, prefix_3, prefix_4, prefix_5, prefix_6
    purity_results = {}
    prefix_stats = {}
    ...
    for length in [2, 3, 4, 5, 6]:
        ...
        for key, col in categorical_targets.items():
            # For each prefix, find the most common value and its percentage (purity)
            puries = []
    ...
    # 4. Generate report showing prefix statistics and build prefix intelligence engine dict
    # We will build a nested dict prefix_stats: length -> prefix -> stats
    for length in [2, 3, 4, 5, 6]:
        prefix_stats[length] = {}
        grouped = df.groupby(f'prefix_{length}')
        for pref, group in grouped:
            stats = {}
            # Calculate for each categorical column
            for key, col in categorical_targets.items():
            ...
            # Confidence score is average confidence across make and model (the primary hierarchy)
    ...
    report_content = []
    report_content.append("# 9-Character VIN Dataset Pattern & Prefix Purity Report\n")
...
if __name__ == "__main__":
    ...
    output_dir = os.path.join(_parent, 'chat_cat_short_vin_09', 'models')
